...
// Place as workers/worker-d1-binding.js and bind a D1 database in your worker (e.g., in wrangler.toml).
    ...
    try {
      ...
      if (request.method === 'GET' && path === '/api/news') {
        const res = await env.DB.prepare(
          'SELECT id, title, body, embed, published_at FROM articles ORDER BY published_at DESC LIMIT 100'
        ).all();
      ...
      if (request.method === 'GET' && m) {
        ...
        const res = await env.DB.prepare(
          'SELECT id, title, body, embed, published_at FROM articles WHERE id = ? LIMIT 1'
        ).bind(id).all();
      ...
      if (request.method === 'POST' && path === '/api/news') {
        ...
        // caller provides id (UUID) or we can create one
        const id = body.id || crypto.randomUUID();
        const published_at = body.published_at || new Date().toISOString();
        await env.DB.prepare(
          'INSERT INTO articles (id, title, body, embed, published_at, created_at) VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)'
        ).bind(id, body.title, body.body, body.embed, published_at).run();
      ...
      if (request.method === 'PUT' && m) {
        ...
        const body = await request.json();
        await env.DB.prepare(
          'UPDATE articles SET title = ?, body = ?, embed = ? WHERE id = ?'
        ).bind(body.title, body.body, body.embed, id).run();
